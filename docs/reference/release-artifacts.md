...
Gradle repository 是 `https://jitpack.io`，`compileOnly` 座標是 `com.github.smile-minecraft:AceLib:v1.2.0`（此 checkout 的原始碼版本為 1.2.0；該座標對應 `v1.2.0` tag，在本機驗證請用 `publishToMavenLocal` 搭配 `com.smile:acelib:1.2.0`）。可直接複製的完整設定與 Paper API dependency 請看[快速開始](../consumer/quickstart.md)。

JitPack 座標 `com.github.smile-minecraft:AceLib:v1.2.0` 對應 `v1.2.0` tag，提供編譯用 API；其 artifact 命名如下：

- `AceLib-v1.2.0.jar`
- `AceLib-v1.2.0-sources.jar`
- `AceLib-v1.2.0-javadoc.jar`

主 JAR 包含 `AceLibApi`、`AceLibApi.AceLibProvider` 與 `AceLibVersion`。POM 座標為 `com.github.smile-minecraft:AceLib:v1.2.0`（對應 v1.2.0 Git tag），沒有 transitive dependencies；Gradle module metadata 要求 Java 25。
...
這會在本機提供 `com.smile:acelib:1.2.0`。它不是 Maven Central 座標，也不應作為一般 plugin 開發者的安裝方式。
