...
AceLib 1.2.0 採用 Java 25，目標伺服器版本是 Paper 與 Folia 26.1.2（已驗證基線）；26.2 系列標記為 VERIFIED-BETA，機器可讀的執行期相容矩陣見 [runtime-compatibility-matrix.json](../reference/runtime-compatibility-matrix.json)。
...
- Paper 與 Folia 26.2 系列經直接驗證（含 Folia 26.2-4 上的真實基岩四項驗證），標記為 **VERIFIED-BETA**：上游仍為 beta，未列為 SUPPORTED。AceLib 的 SUPPORTED 基準仍為 Paper 26.1.2-72 與 Folia 26.1.2-8；未來未知版本列為 UNVERIFIED。詳見[執行期相容矩陣](../reference/runtime-compatibility-matrix.json)。
...
v1.1.0 的基岩功能曾在 Folia 26.2-4 上做過單一功能的實機驗證（Floodgate 2.2.5-SNAPSHOT b140 + Geyser-Spigot 2.11.2-b1232，含真人基岩客戶端的表單操作；詳見 CHANGELOG 的 1.1.0 紀錄）。這是特定組合下針對基岩表單功能的驗證紀錄。自 1.2.0 起，Folia 26.2-4 與 Paper 26.2-120、Folia 26.2-7 經直接驗證標記為 VERIFIED-BETA（見[執行期相容矩陣](../reference/runtime-compatibility-matrix.json)），但上游仍為 beta，未列為 SUPPORTED；排程、上下文與其他模組在 26.2 上的驗證狀態以矩陣為準。評估升級時，仍應以 26.1.2 為 SUPPORTED 基準，並先在獨立測試 server 驗證。
