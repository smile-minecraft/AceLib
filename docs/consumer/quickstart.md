...
AceLib 使用 `compileOnly`，因為執行時會由 server 的 `plugins/AceLib-1.1.0.jar` 提供。JitPack `v1.1.0` 已由 JitPack 成功建置並產出 artifacts。

若 Gradle 找不到 AceLib，先確認 repository URL 是 `https://jitpack.io`，座標的 group 是 `com.github.smile-minecraft`，版本包含 `v`：`v1.1.0`。
...
### `Could not find com.github.smile-minecraft:AceLib:v1.1.0`

檢查是否加入 JitPack、大小寫是否為 `AceLib`，以及版本前面的 `v` 是否保留。不要改用 `com.smile:acelib:1.1.0`；那是 repository 貢獻者在本機執行 `publishToMavenLocal` 後才有的座標。

### Server 顯示 missing dependency

你的 plugin JAR 已放進 `plugins/`，但 AceLib runtime JAR 不在。管理員可依[部署指南](../operator/README.md)從原始碼建立 `build/libs/AceLib-1.1.0.jar`。
...
該範例使用 `mavenLocal()` 與 `com.smile:acelib:1.1.0`，只用於驗證目前 checkout 的程式碼。一般 plugin 專案請使用本頁前面的 JitPack 座標。
